...
dataset_pos=TestbedDataset2(root='data1', dataset=input_f)

print ('alldata len:',dataset_pos.__len__())
#35080

dataset_test_all=dataset_pos
...
arr_models=glob.glob("full_model_out2000.model")
for  model_name in arr_models:
        ...
        fw=open("output_"+input_f.replace('_n_dict','')+"_n.txt",'w')
        for  i in range(len(N)):
              # print (str(G[i])+','+str(P[i])+','+str(N[i][0]))
              fw.write( str(N[i])+','+  str(P[i]) )
              fw.write("\n")
        fw.close()
