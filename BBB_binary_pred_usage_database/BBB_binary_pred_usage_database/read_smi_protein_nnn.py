...
#print(all_dict)
df = pd.read_csv('/home/zhang/Desktop/work/BBB_pred_add/database/'+input_f,sep='\t',header=0)

# 将 DataFrame 转换为字典
# 使用 'A' 列作为键，'B' 列作为值
all_dict = df.set_index('id')['smiles'].to_dict()
...
train_Y_log = np.log10(train_Y)

train_data = TestbedDataset(root='data1', dataset=input_f.replace('.cxsmiles',''), xd=arr_name, y=train_Y_log, smile_graph=smile_graph)
